...
export default function Home() {
  ...
  return (
    <div className="min-h-screen screen-hero relative flex items-center justify-start overflow-hidden font-sans">
      ...
      {view === "satis_ekrani" && (
        <div className="w-full max-w-[98vw] h-[94vh] min-h-[700px] mx-auto flex flex-col md:flex-row gap-10 p-12 shell border b-line rounded-[2.5rem] shadow-deep backdrop-blur-2xl">
          ...
          <div className="flex flex-col w-full md:w-1/2 gap-6 h-full">
            <div className="flex gap-4 items-center shrink-0">
              ...
              <div className="flex-1 flex items-center border b-strong rounded-xl p-5 input-dark relative">
                <svg className="w-10 h-10 t-muted mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 4h4v16H3V4zm6 0h2v16H9V4zm4 0h2v16h-2V4zm4 0h4v16h-4V4z" /></svg>
                <input 
                  type="text" 
                  value={barkodInput}
                  ...
                  className="bg-transparent w-full t-text outline-none placeholder-faint tracking-widest text-lg uppercase"
                  autoFocus
                />
                {aktifSatici && (
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2 border b-glass px-4 py-2 rounded glass">
                    <svg className="w-4 h-4 t-green" fill="currentColor" viewBox="0 0 20 20"><circle cx="10" cy="10" r="10" /></svg>
                    <span className="t-bright text-xs tracking-widest">{aktifSatici}</span>
                  </div>
                )}
              </div>
            </div>
              ...
              {sepetUrunleri.length > 0 ? (
                <div className="flex-1 overflow-auto flex flex-col gap-2">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b-2 b-paper text-[11px] font-bold tracking-widest uppercase">
                        <th className="py-3">Barkod</th>
                        <th className="py-3">Ürün Adı</th>
                        <th className="py-3 text-center">Beden/Renk</th>
                    ...
                    <tbody>
                      {sepetUrunleri.map((urun, idx) => (
                        <tr key={idx} className="border-b b-paper text-sm font-semibold">
                          <td className="py-4">{urun.barkod}</td>
                          <td className="py-4">{urun.isim}</td>
                          <td className="py-4 text-center">{urun.beden} / {urun.renk}</td>
                          <td className="py-4 text-center">{urun.miktar}</td>
                          <td className="py-4 text-right">{formatFiyat(urun.fiyat)} TL</td>
                          <td className="py-4 text-right">{formatFiyat(urun.fiyat * urun.miktar)} TL</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  <div className="mt-auto border-t-2 b-paper pt-6 flex justify-between items-center px-4">
                     <span className="text-base font-bold tracking-widest uppercase">Genel Toplam:</span>
                     <span className="text-4xl font-black">{formatFiyat(sepetGenelToplam)} TL</span>
                  </div>
                </div>
              ) : (
                <div className="flex-1 flex flex-col items-center justify-center">
                  <svg className="w-32 h-32 t-ink-muted mb-6 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M3 4h4v16H3V4zm6 0h2v16H9V4zm4 0h2v16h-2V4zm4 0h4v16h-4V4z" /></svg>
                  <span className="t-ink-muted tracking-widest text-lg font-semibold uppercase">BARKOD OKUTMA ALANI</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col w-full md:w-1/2 justify-between h-full">
              ...
              <div className="grid grid-cols-5 gap-4 mb-6">
                {[
                  ...
                  <button 
                    ...
                    className="flex flex-col items-center justify-center p-5 border b-strong rounded-2xl hover-border-strong hover-glass transition-all group h-36"
                  >
                    <svg className="w-10 h-10 t-muted group-hover:t-text mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={btn.icon} /></svg>
                ...
                <button 
                  ...
                  className="flex flex-col items-center justify-center p-5 border b-purple bg-purple-soft rounded-2xl hover-purple hover-brighten transition-all group h-36"
                >
                  <svg className="w-10 h-10 t-purple group-hover:t-text mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              {[
                ...
                <button 
                  key={btn.id} 
                  onClick={() => handleMenuClick(btn.id)}
                  className="flex flex-col items-center justify-center p-5 border b-strong rounded-2xl hover-border-strong hover-glass transition-all group h-32"
                >
                  <svg className="w-10 h-10 t-muted group-hover:t-text mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={btn.icon} /></svg>
                  <span className="text-xs text-center t-muted group-hover:t-text tracking-widest font-semibold leading-tight uppercase">{btn.label}</span>
                </button>
            ...
            <div className="grid grid-cols-3 gap-6 mt-auto">
              <button 
                ...
                className="flex flex-col items-center justify-center p-8 border b-line rounded-2xl hover-green hover-brighten transition-all group h-48 relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-green-500/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <svg className="w-16 h-16 t-bright group-hover:t-green mb-4 z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                <span className="text-base t-bright group-hover:t-text tracking-widest font-bold z-10 uppercase">NAKİT ÖDEME</span>
              </button>

              <button 
                onClick={() => openOdemeEkrani("Kredi Kartı")} 
                className="flex flex-col items-center justify-center p-8 border b-line rounded-2xl hover-blue hover-brighten transition-all group h-48 relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <svg className="w-16 h-16 t-bright group-hover:t-blue mb-4 z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                <span className="text-base t-bright group-hover:t-text tracking-widest font-bold z-10 uppercase">KREDİ KARTI</span>
              </button>

              <button 
                onClick={() => setAktifModal("hediye_ceki_odeme")} 
                className="flex flex-col items-center justify-center p-8 border b-purple rounded-2xl hover-purple hover-brighten transition-all group h-48 relative overflow-hidden glow-purple"
              >
                <div className="absolute inset-0 bg-purple-500/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <svg className="w-16 h-16 t-purple group-hover:t-purple mb-4 z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" /></svg>
                <span className="text-base t-purple group-hover:t-text tracking-widest font-bold z-10 uppercase">HEDİYE ÇEKİ</span>
              </button>
            </div>
          </div>
        </div>
