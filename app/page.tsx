...
export default function Home() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (loginUser === "adminlume" && loginPass === "umut695418") {
      setIsAdmin(true);
      setLoginError(false);
      setView("home");
      fetchPersoneller();
    } else if (loginUser === "user" && loginPass === "user695418") {
      setIsAdmin(false);
      setLoginError(false);
      setView("home");
      fetchPersoneller();
  ...
  const handleOdemeEkle = async () => {
    ...
    if (yeniKalan <= 0.01) {
      const musteriBilgisi = searchResult ? `${searchResult.isim} ${searchResult.soyisim}` : "Kayıtsız Müşteri";
    ...
    <div className="min-h-screen screen-hero relative flex items-center justify-start overflow-hidden font-sans">
      ...
      {view === "new_customer" && (
        <div className="flex gap-16 pl-16 md:pl-32 items-center w-full max-w-7xl transition-all duration-700 animate-fade-in z-10">
          <div onClick={() => changeView("pos_menu")} className="menu-item flex flex-col items-center gap-6 shrink-0 group">
      ...
      {aktifModal === "hediye_ceki_odeme" && (
        <div className="fixed inset-0 overlay backdrop-blur-md z-50 flex items-center justify-center animate-fade-in p-4">
          <div className="panel border b-purple p-8 rounded-3xl glow-purple w-full max-w-lg relative flex flex-col gap-6">
            <h2 className="t-purple text-lg tracking-widest font-bold border-b b-faint pb-4 uppercase">
              HEDİYE ÇEKİ İLE ÖDEME
            </h2>

            <div className="flex flex-col gap-2">
              <label className="t-muted text-[10px] tracking-widest font-bold uppercase">ÇEK NUMARASI (KOD)</label>
              <input
                type="text"
                ...
                className="field w-full p-4 text-sm uppercase"
                autoFocus
              />
            </div>

            {odemeCekDurumu === "bulundu" && odemeCekBilgi && (
              <div className="bg-green-soft border b-green rounded-xl p-4 flex flex-col gap-2 t-green">
                <span className="text-xs font-bold tracking-widest uppercase">ÇEK DOĞRULANDI - GEÇERLİ BAKİYE:</span>
                <span className="text-2xl font-black t-text">{formatFiyat(odemeCekBilgi.cek_fiyat_tutari)} TL</span>
              </div>
            )}

            {odemeCekDurumu === "bulunamadi" && (
              <div className="bg-red-soft border b-red rounded-xl p-4 t-red text-xs tracking-widest font-bold uppercase">
            ...
            <div className="flex gap-4 mt-2">
              ...
              {odemeCekDurumu !== "bulundu" ? (
                <button
                  ...
                  className={`flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase transition-all ${odemeCekKodu ? 'btn-solid bg-purple glow-purple' : 'surface-3 t-faint cursor-not-allowed'}`}
                >
                  Sorgula
                </button>
              ) : (
                <button
                  ...
                  className="btn-solid bg-green flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase glow-green"
                >
                  Ödemeyi Tamamla
                </button>
              )}
      ...
      {view === "satis_ekrani" && (
        <>
          {!aktifSatici && (
            <div className="fixed inset-0 overlay backdrop-blur-md z-50 flex items-center justify-center animate-fade-in">
              <div className="panel border b-line p-12 rounded-3xl shadow-deep max-w-6xl w-full">
                <h2 className="t-text text-2xl tracking-widest font-light mb-8 text-center border-b b-faint pb-4 uppercase">
                  ...
                  {personeller.length > 0 ? (
                    personeller.map((personel) => (
                      <button
                        ...
                        className="py-6 px-4 border b-strong rounded-xl hover-glass hover-border-strong transition-all t-text tracking-widest font-bold text-sm uppercase flex flex-col gap-2 items-center"
                      >
                        <span className="t-muted text-[10px] input-dark px-3 py-1 rounded-full">{personel.personel_kodu}</span>
                        <span>{personel.isim_soyisim}</span>
                      </button>
                    ))
                  ) : (
                    <div className="col-span-full text-center t-faint py-8 tracking-widest text-sm uppercase">SİSTEMDE TANIMLI PERSONEL BULUNAMADI. LÜTFEN DESTEK KISMINDAN PERSONEL TANIMLAYIN.</div>
                  )}
                </div>
              </div>
            </div>
          ...
          {aktifModal !== "none" && aktifModal !== "hediye_ceki_odeme" && aktifModal !== "destek_personel" && aktifModal !== "yonetim_sicil" && aktifModal !== "destek_menu" && aktifModal !== "yonetim_menu" && aktifModal !== "yonetim_hediye_ceki" && aktifModal !== "yonetim_hediye_ceki_onay" && aktifModal !== "odeme_ekrani" && (
            <div className="fixed inset-0 overlay-soft backdrop-blur-sm z-40 flex items-center justify-center animate-fade-in p-4">

              {aktifModal === "fiyat_sorgula" && (
                <div className="panel border b-blue-soft p-8 rounded-2xl glow-blue w-full max-w-lg relative">
                  <h2 className="t-blue text-lg tracking-widest font-bold mb-6 border-b b-faint pb-4 uppercase">FİYAT SORGULAMA</h2>
                  <input type="text" placeholder="BARKOD OKUTUNUZ..." className="field w-full p-4 mb-6 tracking-widest" autoFocus />

                  <div className="text-center t-faint py-6 tracking-widest text-sm font-bold uppercase">SİSTEMDE ÜRÜN BULUNAMADI.</div>

                  <button onClick={() => setAktifModal("none")} className="btn-ghost w-full py-4 rounded-xl font-bold tracking-widest text-xs uppercase">KAPAT</button>
                </div>
              )}

              {aktifModal === "urun_sorgula" && (
                <div className="panel border b-teal-soft p-8 rounded-2xl glow-teal w-full max-w-2xl relative">
                  <h2 className="t-teal text-lg tracking-widest font-bold mb-6 border-b b-faint pb-4 uppercase">ÜRÜN VE STOK SORGULAMA</h2>
                  <input type="text" placeholder="BARKOD OKUTUNUZ..." className="field w-full p-4 mb-4 tracking-widest" autoFocus />
              ...
              {aktifModal === "iskonto" && (
                <div className="panel border b-orange-soft p-8 rounded-2xl glow-orange w-full max-w-md relative">
                  <h2 className="t-orange text-lg tracking-widest font-bold mb-6 border-b b-faint pb-4 uppercase">İSKONTO (İNDİRİM) UYGULA</h2>
                  <div className="flex gap-4 mb-6">
                    <button className="flex-1 py-3 bg-orange-soft2 border b-orange rounded-xl t-text font-bold text-xs tracking-widest">YÜZDE (%)</button>
                    <button className="flex-1 py-3 btn-ghost rounded-xl t-muted font-bold text-xs tracking-widest">TUTAR (TL)</button>
                  </div>
                  <input type="number" placeholder="İNDİRİM DEĞERİNİ GİRİNİZ..." className="field w-full p-4 mb-6 tracking-widest" autoFocus />
                  <div className="flex gap-4">
                    <button onClick={() => setAktifModal("none")} className="btn-ghost flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase">İPTAL</button>
                    <button onClick={() => setAktifModal("none")} className="btn-solid bg-orange flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase glow-orange">UYGULA</button>
                  </div>
                </div>
              )}

              {aktifModal === "satis_iptal" && (
                <div className="panel border b-red p-8 rounded-2xl glow-red w-full max-w-md relative text-center">
                  <svg className="w-16 h-16 t-red mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                  <h2 className="t-text text-xl tracking-widest font-bold mb-4 uppercase">FİŞ İPTALİ</h2>
                  <p className="t-muted font-light text-sm tracking-wide mb-8">Mevcut satışı tamamen iptal etmek ve sepeti boşaltmak istediğinize emin misiniz? (Düşülen stoklar depoya geri eklenecektir)</p>
                  <div className="flex gap-4">
                    <button onClick={() => setAktifModal("none")} className="btn-ghost flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase">HAYIR, VAZGEÇ</button>
                    <button onClick={handleSatisIptal} className="btn-solid bg-red flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase glow-red">EVET, İPTAL ET</button>
                  </div>
                </div>
              )}

              {aktifModal === "urun_iptal" && (
                <div className="panel border b-yellow-soft p-8 rounded-2xl glow-yellow w-full max-w-md relative">
                  <h2 className="t-yellow text-lg tracking-widest font-bold mb-6 border-b b-faint pb-4 uppercase">ÜRÜN ÇIKART (İPTAL)</h2>
                  <p className="t-muted font-light text-sm tracking-wide mb-4 text-center">Sepetten çıkartmak istediğiniz ürünün barkodunu okutunuz.</p>
                  <input type="text" placeholder="BARKOD OKUTUNUZ..." className="field w-full p-4 mb-6 tracking-widest text-center" autoFocus />
                  <button onClick={() => setAktifModal("none")} className="btn-ghost w-full py-4 rounded-xl font-bold tracking-widest text-xs uppercase">KAPAT / VAZGEÇ</button>
                </div>
              )}

              {aktifModal === "satisi_beklet" && (
                <div className="panel border b-cyan p-8 rounded-2xl glow-cyan w-full max-w-md relative text-center">
                  <svg className="w-16 h-16 t-cyan mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  <h2 className="t-text text-xl tracking-widest font-bold mb-4 uppercase">SATIŞI BEKLET (ASKIYA AL)</h2>
                  <p className="t-muted font-light text-sm tracking-wide mb-6">Mevcut sepeti beklemeye almak istiyor musunuz? İşleme daha sonra <strong className="t-text">Askıdan Çağır</strong> menüsünden devam edebilirsiniz.</p>
                  <input type="text" placeholder="SEPET İÇİN BİR NOT GİRİNİZ (OPSİYONEL)..." className="field w-full p-4 mb-6 tracking-widest text-center text-xs" />
                  <div className="flex gap-4">
                    <button onClick={() => setAktifModal("none")} className="btn-ghost flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase">İPTAL</button>
                    <button onClick={() => setAktifModal("none")} className="btn-solid bg-cyan flex-1 py-4 rounded-xl font-bold tracking-widest text-xs uppercase glow-cyan">ASKIYA AL</button>
                  </div>
                </div>
              )}

              {aktifModal === "askidan_cagir" && (
                <div className="panel border b-indigo p-8 rounded-3xl glow-indigo w-full max-w-6xl relative h-[85vh] flex flex-col">

                  <div className="flex justify-between items-center mb-6 border-b b-faint pb-4 shrink-0">
                    <h2 className="t-indigo text-xl tracking-widest font-bold uppercase">BEKLEYEN (ASKIDAKİ) SEPETLER</h2>
                    <button onClick={() => setAktifModal("none")} className="px-6 py-2 bg-red-soft t-red hover-red rounded-xl border b-red transition-all text-xs font-bold tracking-widest uppercase">
                      KAPAT
                    </button>
                  </div>
                  ...
                  <div className="mb-6 shrink-0 relative">
                    <svg className="w-5 h-5 t-faint absolute left-4 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    <input type="text" placeholder="SEPET NOTU VEYA MÜŞTERİ ARA..." className="field w-full pl-12 pr-4 py-4 text-xs uppercase" />
                  </div>

                  <div className="paper rounded-2xl flex-1 overflow-auto border b-paper inset-paper p-1 flex items-center justify-center">
                    <span className="t-ink-muted font-bold uppercase tracking-widest text-sm">BEKLEYEN SEPET BULUNAMADI.</span>
                  </div>
              ...
              {aktifModal === "kampanya" && (
                <div className="panel border b-purple p-8 rounded-2xl glow-purple w-full max-w-md relative text-center">
                  <div className="text-6xl mb-6">🎵</div>
                  <h2 className="t-text text-2xl tracking-widest font-bold mb-4 uppercase">ÇOK FAZLA FERDİ TAYFUR DİNLEDİĞİNİ TESPİT ETTİK</h2>
                  <div className="bg-purple-soft border b-purple rounded-xl p-6 mb-8">
                    <p className="t-purple font-light text-lg tracking-wide">
                      "Umut'u tanıyorsanız size özel <strong className="t-text font-bold">3 AL 5 ÖDE</strong> kampanyası aktif edilmiştir! 🤫💸"
                    </p>
                  </div>
                  <button onClick={() => setAktifModal("none")} className="btn-solid bg-purple w-full py-4 rounded-xl font-bold tracking-widest text-xs uppercase glow-purple">
                    ONAYLA
                  </button>
                </div>
              )}
          ...
          <div className="w-full max-w-[1500px] h-[88vh] min-h-[640px] mx-auto flex flex-col md:flex-row gap-8 p-10 shell border b-line rounded-3xl shadow-panel backdrop-blur-xl">

            <div className="flex flex-col w-full md:w-1/2 gap-4">
              <div className="flex gap-4 items-center">
                <button
                  onClick={() => { changeView("pos_menu"); setAktifSatici(""); }}
                  className="p-3 border b-strong rounded-xl hover-surface transition-colors flex items-center justify-center t-text"
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                </button>

                <div className="flex-1 flex items-center border b-strong rounded-xl p-3 input-dark relative">
              ...
              <div className="paper t-ink flex-1 rounded-2xl min-h-[520px] flex flex-col relative inset-paper p-4 overflow-hidden">
                {sepetUrunleri.length > 0 ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center">
                    <svg className="w-24 h-24 t-ink-muted mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M3 4h4v16H3V4zm6 0h2v16H9V4zm4 0h2v16h-2V4zm4 0h4v16h-4V4z" /></svg>
                    <span className="t-ink-muted tracking-widest text-sm font-semibold uppercase">BARKOD OKUTMA ALANI</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col w-full md:w-1/2 justify-between">
              ...
              <div className="grid grid-cols-5 gap-3 mb-6">
                {[
                  ...
                  <button
                    ...
                    className="flex flex-col items-center justify-center p-4 border b-strong rounded-xl hover-border-strong hover-glass transition-all group h-28"
                  >
                    <svg className="w-7 h-7 t-muted group-hover:t-text mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={btn.icon} /></svg>
                    <span className="text-xs text-center t-muted group-hover:t-text tracking-widest font-semibold leading-tight uppercase">{btn.label}</span>
                  </button>
                ))}

                <button
                  onClick={() => setAktifModal("kampanya")}
                  className="flex flex-col items-center justify-center p-4 border b-purple bg-purple-soft rounded-xl hover-purple hover-brighten transition-all group h-28"
                >
                  <svg className="w-7 h-7 t-purple group-hover:t-text mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                  <span className="text-xs text-center t-purple group-hover:t-text tracking-widest font-semibold leading-tight uppercase">KAMPANYA</span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3 mb-6">
                {[
                  ...
                  <button
                    key={btn.id}
                    onClick={() => handleMenuClick(btn.id)}
                    className="flex flex-col items-center justify-center p-4 border b-strong rounded-xl hover-border-strong hover-glass transition-all group h-24"
                  >
                    <svg className="w-7 h-7 t-muted group-hover:t-text mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={btn.icon} /></svg>
                    <span className="text-xs text-center t-muted group-hover:t-text tracking-widest font-semibold leading-tight uppercase">{btn.label}</span>
                  </button>
              ...
              <div className="grid grid-cols-3 gap-4 mt-auto">
                <button
                  ...
                  className="flex flex-col items-center justify-center p-6 border b-line rounded-xl hover-green hover-brighten transition-all group h-32 relative overflow-hidden"
                >
                  <svg className="w-10 h-10 t-bright group-hover:t-green mb-3 z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                  <span className="text-xs t-bright group-hover:t-text tracking-widest font-semibold z-10 uppercase">NAKİT ÖDEME</span>
                </button>

                <button
                  onClick={() => openOdemeEkrani("Kredi Kartı")}
                  className="flex flex-col items-center justify-center p-6 border b-line rounded-xl hover-blue hover-brighten transition-all group h-32 relative overflow-hidden"
                >
                  <svg className="w-10 h-10 t-bright group-hover:t-blue mb-3 z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                  <span className="text-xs t-bright group-hover:t-text tracking-widest font-semibold z-10 uppercase">KREDİ KARTI</span>
                </button>

                <button
                  onClick={() => setAktifModal("hediye_ceki_odeme")}
                  className="flex flex-col items-center justify-center p-6 border b-purple rounded-xl hover-purple hover-brighten transition-all group h-32 relative overflow-hidden glow-purple"
                >
                  <svg className="w-10 h-10 t-purple group-hover:t-purple mb-3 z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" /></svg>
                  <span className="text-xs t-purple group-hover:t-text tracking-widest font-semibold z-10 uppercase">HEDİYE ÇEKİ</span>
                </button>
              </div>
            </div>
